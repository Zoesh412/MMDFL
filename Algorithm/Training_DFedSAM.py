...
class LocalUpdate_DFedSAM(object):
    ...
    def train(self, client, round):
        ...
        if self.args.optimizer == 'sgd':
            optimizer = SAM(net.parameters(), base_optimizer=torch.optim.SGD ,lr=self.args.lr*(self.args.lr_decay**round),
...
def DFedSAM(args, net_glob, dataset_train, dataset_test, dict_users):
    ...
    for iter in range(args.epochs):
        ...
        for idx in tqdm(range(args.num_users)):
            local = LocalUpdate_DFedSAM(args=args, dataset=dataset_train)
        ...
        for idx in range(args.num_users):
            w_locals = []
            lens = []
            t = get_training_time(client_type_list[idx])*2
