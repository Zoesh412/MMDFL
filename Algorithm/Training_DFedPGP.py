...
def DFedPGP(args, share_net_glob, private_net_glob, dataset_train, dataset_test, dict_users):
    ...
    for iter in range(args.epochs):
        ...
        for idx in range(args.num_users):
            w_locals = []
            lens = []
            t = get_training_time(client_type_list[idx]) * 1.2 #(5 private_epoch + 1 share_epoch) / 5 epoch
            for nb_idx in range(args.num_users):
                if Adjacency_matrix[idx][nb_idx] == 1:
                    ...
                    lens.append(1)
                    t = t + get_communication_time(NetWork_type[idx][nb_idx])*bandwith_ratio
                    round_comm = round_comm + (share_net_size * 8) / (1024 * 1024)  # MB
